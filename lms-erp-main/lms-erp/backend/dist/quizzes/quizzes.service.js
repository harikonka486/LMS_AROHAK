...
let QuizzesService = class QuizzesService {
    ...
    async create(courseId, body) {
        ...
        try {
            ...
            await conn.query('INSERT INTO quizzes (id,course_id,title,description,passing_score,order_num) VALUES (?,?,?,?,?,?)', [
                qid,
                courseId,
                title,
                description || null,
            ...
            for (const q of questions || []) {
                await conn.query('INSERT INTO questions (id,quiz_id,text,options,correct_answer,explanation) VALUES (?,?,?,?,?,?)', [
                    (0, uuid_1.v4)(),
                    qid,
                    q.text,
    ...
    async submit(quizId, answers, userId) {
      ...
      `, [userId, courseId]));
            if (Number(passedCount) >= Number(total)) {
                ...
                const [[courseForCert]] = (await this.db.query('SELECT title FROM courses WHERE id=?', [courseId]));
                await this.db.query('INSERT IGNORE INTO certificates (id,user_id,course_id,certificate_number,score,course_title_snapshot) VALUES (?,?,?,?,?,?)', [(0, uuid_1.v4)(), userId, courseId, certNum, Math.round(score * 100) / 100, courseForCert?.title || null]);
            }
