...
let LessonsService = class LessonsService {
    ...
    async create(sectionId, body, file) {
        ...
        await this.db.query('INSERT INTO lessons (id,title,description,video_url,video_file,duration,is_free,order_num,section_id) VALUES (?,?,?,?,?,?,?,?,?)', [id, title, description || null, video_url || null, video_file, duration || null, is_free ? 1 : 0, Number(count) + 1, sectionId]);
        const [[lesson]] = await this.db.query('SELECT * FROM lessons WHERE id=?', [id]);
        return lesson;
    }
    async update(id, body, file) {
        ...
        const vals = [title, description, video_url, duration, is_free ? 1 : 0];
        if (video_file) {
