...
let AuthService = class AuthService {
    ...
    async register(body) {
        ...
        await this.db.query('INSERT INTO email_verification_tokens (id, user_id, token, expires_at) VALUES (?,?,?, DATE_ADD(NOW(), INTERVAL 24 HOUR))', [(0, uuid_1.v4)(), id, verifyToken]);
        this.mail.sendWelcome(email, name, verifyToken);
        return { token: this.sign(id), user };
    }
    async verifyEmail(token) {
        const [[record]] = (await this.db.query('SELECT * FROM email_verification_tokens WHERE token=?', [token]));
        if (!record)
            throw new common_1.BadRequestException('Invalid verification link');
        if (record.used) {
            const [[user]] = (await this.db.query('SELECT is_email_verified FROM users WHERE id=?', [record.user_id]));
            if (user?.is_email_verified) {
                return { message: 'Email already verified!' };
            }
            throw new common_1.BadRequestException('This verification link has already been used');
        }
        const [[valid]] = (await this.db.query('SELECT id FROM email_verification_tokens WHERE token=? AND used=0 AND expires_at > NOW()', [token]));
        if (!valid)
    ...
    async login(body) {
        ...
        if (!user.is_email_verified) {
            throw new common_1.UnauthorizedException('Please verify your email address before logging in. Check your inbox for the verification link.');
        }
