...
let CoursesService = class CoursesService {
    ...
    async create(body, file, userId) {
        const { title, description, price = 0, level = 'beginner', language = 'English', categoryId, passing_score = 70, video_url, sharepoint_video_url, } = body;
        const thumbnail = file ? `/uploads/thumbnails/${file.filename}` : null;
        const id = (0, uuid_1.v4)();
        await this.db.query('INSERT INTO courses SET id=?, title=?, description=?, price=?, level=?, language=?, thumbnail=?, instructor_id=?, category_id=?, passing_score=?, video_url=?, sharepoint_video_url=?', [id, title, description, price, level, language, thumbnail, userId, categoryId || null, passing_score, video_url || null, sharepoint_video_url || null]);
    ...
    async update(id, body, file, user) {
        ...
        const thumbnail = file
            ...
            : course.thumbnail;
        const { title, description, price, level, language, categoryId, passing_score, video_url, sharepoint_video_url, } = body;
        await this.db.query('UPDATE courses SET title=?,description=?,price=?,level=?,language=?,thumbnail=?,category_id=?,passing_score=?,video_url=?,sharepoint_video_url=?,updated_at=NOW() WHERE id=?', [
            title || course.title,
