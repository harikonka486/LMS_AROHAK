...
let CertificatesService = class CertificatesService {
    ...
    async findAll() {
        const [rows] = (await this.db.query(`SELECT cert.*,
    ...
    async findMy(userId) {
       ...
       JOIN users u ON u.id = cert.user_id
       LEFT JOIN users inst ON inst.id = c.instructor_id
       WHERE cert.user_id=? ORDER BY cert.issued_at DESC`, [userId]));
        return rows;
    }
    async verify(number) {
        const [[cert]] = (await this.db.query(`SELECT cert.*,
              COALESCE(c.title, cert.course_title_snapshot, 'Course Deleted') AS course_title,
              u.name AS user_name, u.employee_id
       FROM certificates cert
       LEFT JOIN courses c ON c.id = cert.course_id
       JOIN users u ON u.id = cert.user_id
       WHERE cert.certificate_number=?`, [number]));
    ...
    async delete(id) {
        const [[cert]] = (await this.db.query('SELECT id FROM certificates WHERE id=?', [id]));
        if (!cert)
