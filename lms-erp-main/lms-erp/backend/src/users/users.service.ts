...
export class UsersService {
  ...
  async getStats() {
    const [[{ totalCourses }]] = (await this.db.query(
      'SELECT COUNT(*) AS totalCourses FROM courses',
    )) as any;
    const [[{ totalEnrollments }]] = (await this.db.query(
      'SELECT COUNT(*) AS totalEnrollments FROM enrollments',
    )) as any;
    const [[{ completedEnrollments }]] = (await this.db.query(
      "SELECT COUNT(*) AS completedEnrollments FROM enrollments WHERE status='completed'",
    )) as any;
    const [[{ totalCertificates }]] = (await this.db.query(
